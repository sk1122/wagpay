...
const useInvoices = () => {
	...
	async function getInvoices(cursor?: number) {
		if(!cursor) {
			const data = await fetch('http://localhost:2000/api/invoices/', {
				headers: {
			...
			const data = await fetch(`http://localhost:2000/api/invoices?cursor=${cursor.toString()}`, {
				headers: {
					'bearer-token': supabase.auth.session()?.access_token as string,
				},
			})
	...
	async function createInvoices(invoice: Invoice) {
		var data = await fetch('http://localhost:2000/api/invoices/', {
			method: 'POST',
