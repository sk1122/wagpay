...
const useProducts = () => {
	...
	async function getProducts(cursor?: number) {
		if(!cursor) {
			const data = await fetch('https://wagpay.club/api/products/all/', {
				headers: {
			...
			const data = await fetch(`https://wagpay.club/api/products/all?cursor=${cursor.toString()}`, {
				headers: {
					'bearer-token': supabase.auth.session()?.access_token as string,
				},
			})
	...
	async function total_sold() {
		var data = await fetch('https://wagpay.club/api/products/total_sold', {
			headers: {
				'bearer-token': supabase.auth.session()?.access_token as string,
	...
	async function createProducts(page: Page) {
		var data = await fetch('https://wagpay.club/api/products/', {
			method: 'POST',
