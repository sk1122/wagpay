...
app.post("/store", [verifyUser], async (req: Request, res: Response) => {
  ...
  const { data, error } = await supabase.from("Store").insert([store]);
    ...
    if (typeof req.body.payer === "object") {
      ...
      if (!payer || error || payer?.length === 0) {
        res
          .status(401)
          .send(
            "Payer does not exists or Payer data has some issues " +
...
app.post(
  "/store/:store/invoices/:invoice/button",
  async (req: Request, res: Response) => {
    const store_id = req.params["store"];
    const invoice_id = req.params["invoice"];
    ...
    const button_id = await create_unique_button_id(
      store_id,
      invoice_id,
      data[0].created_at
    );
    ...
    let data_btn = await supabase.from("Payment Button").insert([button_data]);

    if (!data_btn.data || data_btn.error || data_btn.data?.length === 0) {
      res
        .status(400)
        .send("User was not created " + JSON.stringify(data_btn.error));
      return;
    }
